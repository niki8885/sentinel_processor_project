...
! fill_edges_arr
  ...
  subroutine fill_edges_arr(arr, mask, n_times, npix, base)
    ...
    do t = 1, n_times
      if (mask(ix(base,t,npix)) /= 0) then; v = arr(ix(base,t,npix)); exit; end if
    end do
    do t = 1, n_times
...
! fill_edges_buf
  ...
  subroutine fill_edges_buf(buf, mask, n_times, npix, base)
    ...
    v = buf(1)
    do t = 1, n_times
      if (mask(ix(base,t,npix)) /= 0) then; v = buf(t); exit; end if
    end do
    do t = 1, n_times
      if (mask(ix(base,t,npix)) /= 0) exit
      buf(t) = v
...
! METHOD 0 -- LINEAR
  ...
  subroutine fill_linear(arr, mask, n_times, npix, base)
    ...
    if (valid_count(mask, n_times, npix, base) == 0) then
      do t = 1, n_times; arr(ix(base,t,npix)) = NODATA; end do; return
    end if
...
! METHOD 1 -- SAVITZKY-GOLAY
  ...
  subroutine fill_savgol(arr, mask, n_times, npix, base, window)
    ...
    do t = 1, n_times; tmp(t) = arr(ix(base,t,npix)); end do
...
! METHOD 2 -- PCHIP CUBIC HERMITE
  ...
  subroutine fill_pchip(arr, mask, n_times, npix, base)
    ...
    do t = 1, n_times; out(t) = arr(ix(base,t,npix)); end do

    do t = 1, n_times
      if (mask(ix(base,t,npix)) /= 0) cycle
      if (real(t,c_double) <= xv(1))  then; out(t) = yv(1);  cycle; end if
      if (real(t,c_double) >= xv(nv)) then; out(t) = yv(nv); cycle; end if

      lo = 1; hi = nv
      do while (hi-lo > 1)
        k = (lo+hi)/2
        if (real(t,c_double) < xv(k)) then; hi = k; else; lo = k; end if
      end do

      h = xv(hi) - xv(lo)
      if (abs(h) < 1.0d-12) then; out(t) = yv(lo); cycle; end if
  ...
  subroutine fill_ets(arr, mask, n_times, npix, base)
    ...
    do ia = 1, 5
      do ib = 1, 5
        ...
        if (sse < best_sse) then; best_sse = sse; alpha = a_try; beta = b_try; end if
      end do
    end do
