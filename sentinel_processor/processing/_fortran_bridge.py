...
def _ms_to_fortran(ms: np.ndarray) -> tuple[ctypes.POINTER, np.ndarray, int, int, int]:
    if ms.ndim != 3:
        raise ValueError(f"ms must be 3-D (n_bands, rows, cols), got shape {ms.shape}")
    n_bands, ms_rows, ms_cols = ms.shape
    ms_f = np.asfortranarray(ms.transpose(1, 2, 0), dtype=np.float64)
    ptr = ms_f.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    return ptr, ms_f, n_bands, ms_rows, ms_cols
...
def _allocate_out(rows: int, cols: int, n_bands: int) -> tuple[ctypes.POINTER, np.ndarray]:
    ...
    ptr   = out_f.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    return ptr, out_f
...
def pansharpen(
    ...
    if pan.ndim != 2:
        raise ValueError(f"pan must be 2-D (rows, cols), got shape {pan.shape}")
    ...
    if algorithm == "ihs" and n_bands != 3:
        raise ValueError(f"IHS pansharpening requires exactly 3 bands, got {n_bands}")
