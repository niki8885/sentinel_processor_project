...
_LIB_NAME = (
    "libsentinel_indices.dll" if sys.platform == "win32" else "libsentinel_indices.so"
)
...
def _get_lib() -> ctypes.CDLL:
    ...
    for name in _two_band_subs:
        fn = getattr(lib, name)
        fn.restype = None

    for name in ["compute_ndvi", "compute_savi", "compute_ndwi",
                 "compute_mndwi", "compute_ndbi", "compute_nbr",
                 "compute_ndsi", "compute_cig"]:
        getattr(lib, name).argtypes = [
            _PTR_DBL, _PTR_DBL, ctypes.c_int, _PTR_DBL
        ]

    for name in ["compute_evi", "compute_arvi"]:
        getattr(lib, name).restype = None
        getattr(lib, name).argtypes = [
            _PTR_DBL, _PTR_DBL, _PTR_DBL, ctypes.c_int, _PTR_DBL
        ]
...
def _prepare(*arrays) -> tuple[list[np.ndarray], int]:
    out = [np.asarray(a, dtype=np.float64).flatten() for a in arrays]
    sizes = {a.size for a in out}
    if len(sizes) != 1:
        raise ValueError(f"All band arrays must have the same number of elements; got sizes {sizes}")
    return out, out[0].size


def _c(arr: np.ndarray) -> ctypes.POINTER:  # noqa: ANN001
