...
class TestPlotRgb:
    ...
    def test_mode_a_returns_figure(self, three_band_tif):
        """Mode A: visual file, no band args."""
...
class TestPlotGrid:
    ...
    def test_per_panel_colorscale(self, single_band_tif):
        ...
        cs_hot = fig_hot.data[0].colorscale
        cs_default = fig_default.data[0].colorscale
        assert cs_hot != cs_default

    def test_same_file_cached(self, single_band_tif):
        """Referencing same file multiple times must not raise."""
        panels = [
            {"file": str(single_band_tif), "label": "A"},
            {"file": str(single_band_tif), "label": "B"},
        ]
...
class TestPlotMask:
    ...
    def test_second_trace_is_pie(self, scl_tif):
        fig = plot_mask(str(scl_tif))
        assert isinstance(fig.data[1], go.Pie)
    ...
    def test_default_bad_classes_applied(self, scl_tif):
        """SCL=8 (cloud) is in DEFAULT_BAD_CLASSES → those rows should be bad."""
        _, mask = plot_mask(str(scl_tif), return_mask=True)
        cloud_rows = mask[2:8, :]  # rows 2-7 have SCL=8
        assert cloud_rows.all()  # all flagged as bad

    def test_clear_rows_not_bad(self, scl_tif):
        _, mask = plot_mask(str(scl_tif), return_mask=True)
        clear_rows = mask[10:, :]  # rows 10+ have SCL=4 (vegetation)
        assert clear_rows.sum() == 0

    def test_custom_bad_classes(self, scl_tif):
        """Only flag class 0 (nodata)."""
